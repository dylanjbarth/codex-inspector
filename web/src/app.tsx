...
export function App() {
  ...
  const message = setup ? 'Install and enable the matching Codex Inspector plugin, then review its seven hooks.' : incompatible ? 'The installed Codex host, CLI, plugin, or source format is outside the supported demo contract.' : degraded ? 'A compatibility, source, or hook diagnostic requires attention.' : status.index.state === 'current' ? 'Every inventoried supported source has reached the current checkpoint.' : 'Inspector is indexing supported history in reverse chronological order.'
