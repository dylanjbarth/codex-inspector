...
function SessionView({route,revision}:{route:Route;revision:number}){
  ...
  const renderBranches=(parentSessionId:string,depth=0,path=new Set<string>()):React.ReactNode=>relations.filter(item=>item.parentSessionId===parentSessionId).sort((a,b)=>a.ordinal-b.ordinal).map(relation=>{if(path.has(relation.childSessionId))return null;const spawnTurn=(turnsBySession.get(parentSessionId)||[]).find(turn=>turn.turnId===relation.spawnTurnId),node=nodeBySession.get(relation.childSessionId),nextPath=new Set(path).add(relation.childSessionId),isCollapsed=collapsed.has(relation.childSessionId),offset=depth?28:Math.min(420,32+(spawnTurn?.ordinal||0)*76);return <section className={`agent-turn-branch${isCollapsed?' collapsed':''}`} style={{'--branch-offset':`${offset}px`} as React.CSSProperties} key={`${relation.parentSessionId}:${relation.childSessionId}:${relation.edgeKind}`}><header><span>↳ {spawnTurn?`Spawned by ${turnName(spawnTurn)}`:'Attached descendant'} · <b>{relation.childSessionId}</b> · {tokens(node?.inclusiveTokens)} downstream</span><button type="button" aria-expanded={!isCollapsed} onPointerDown={event=>event.stopPropagation()} onClick={()=>setCollapsed(value=>{const next=new Set(value);if(next.has(relation.childSessionId))next.delete(relation.childSessionId);else next.add(relation.childSessionId);return next})}>{isCollapsed?'Expand branch':'Collapse branch'}</button></header>{!isCollapsed&&<div className="agent-branch-body"><TurnLane sessionId={relation.childSessionId}/>{renderBranches(relation.childSessionId,depth+1,nextPath)}</div>}</section>})
