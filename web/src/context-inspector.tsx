...
export function ContextInspector({revision,available,onApply}:Props){
  ...
  return <main className="inspector-shell"><header><div><span className="eyebrow">SOURCE-BACKED TRACE</span><h1>Context Inspector</h1></div><a className="quiet-link" href="/">Token &amp; Capacity</a></header>
...
function SessionView({route,revision}:{route:Route;revision:number}){
  ...
  React.useEffect(()=>{let live=true;if(!route.turnId){setLedger(null);return}void (async()=>{try{let page=await fetchLedger(route.sessionId!,route.turnId!,revision);const items=[...page.items];let cursor=page.nextCursor;while(cursor&&items.length<10000){page=await fetchLedger(route.sessionId!,route.turnId!,revision,cursor);items.push(...page.items);cursor=page.nextCursor}if(live)setLedger({...page,items,nextCursor:cursor})}catch(e){if(live){setLedger(null);setError((e as Error).message)}}})();return()=>{live=false}},[route.sessionId,route.turnId,revision])
  ...
  return <section className="session-view"><div className="inspector-breadcrumb"><button onClick={()=>navigate('/context')}>Discovery</button><span>/</span><strong>{map.rootSessionId}</strong>{route.turnId&&<><span>/</span><span>{route.turnId}</span></>}</div>
...
function TurnInspector({rootId,turnId,eventId,evidenceId,revision,ledger}:{rootId:string;turnId:string;eventId?:string;evidenceId?:string;revision:number;ledger:LedgerPage|null}){
  ...
  return <div className="turn-focus"><section className="ledger"><div><span className="section-label">CHRONOLOGICAL EVENT LEDGER</span><h2>Full recorded turn</h2><p>{ledger.items.length} source-backed events. Select one to inspect its exact record.</p></div><ol>{ledger.items.map((item,index)=><li key={item.eventId}><article ref={item.eventId===eventId?eventRef:undefined} className={item.eventId===eventId?'selected':''}><button onClick={()=>navigate(`/context/${encodeURIComponent(rootId)}/turn/${encodeURIComponent(turnId)}/event/${encodeURIComponent(item.eventId)}?evidence=${encodeURIComponent(item.evidenceId)}`)}><span>{String(index+1).padStart(2,'0')}</span><strong>{kindLabel(item.kind)}</strong><time>{new Date(item.observedAt).toLocaleTimeString()}</time></button>{(item.kind==='compacted'||item.kind==='context_compacted')&&<small>First-class exact recorded compaction event; surrounding ledger usage remains visible.</small>}</article></li>)}</ol></section>
