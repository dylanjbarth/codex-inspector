...
func TestMetadataAtomicUserOnlyRoundTrip(t *testing.T) {
	run := t.TempDir()
	want := Metadata{InstanceID: "fake-instance", PID: 123, Port: 4567, ProtocolVersion: 1, AccessToken: "secret", FragmentToken: "fragment", StartedAt: time.Unix(1, 0).UTC()}
	...
	if got.InstanceID != want.InstanceID || got.AccessToken != want.AccessToken {
		t.Fatalf("got %+v", got)
	}
