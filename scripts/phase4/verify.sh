#!/bin/sh
set -eu
export GOCACHE="${GOCACHE:-/tmp/codex-inspector-phase4-gocache}"
...
go vet ./...
go test ./...
go test -race ./internal/evidence ./internal/inspector ./internal/server
go build ./...
...
shellcheck plugin/codex-inspector/hooks/inspector-hook.sh scripts/phase0/*.sh scripts/phase1/*.sh scripts/phase2/*.sh scripts/phase3/*.sh scripts/phase4/*.sh
